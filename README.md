...
Displays the current word and remaining lives.
Accepts a letter from the player.
Checks whether the letter has already been guessed.
If the guess is new, it is stored in a list of previous guesses.
The program searches the secret word for matching letters.
Correct guesses reveal the corresponding positions in the hidden word.
Incorrect guesses reduce the player's remaining lives.
The Hangman drawing is updated based on the number of lives remaining.
